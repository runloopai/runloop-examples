...
from it start ready, with no per-run install. Building one takes ~40s, so this is
a one-time step: by default it reuses the newest already-built `kernel-browser`
blueprint and only builds when none exists. `run` then creates devboxes from the
blueprint by name.
"""
...
def create_kernel_blueprint(runloop: RunloopSDK | None = None, *, rebuild: bool = False) -> str:
    """Return a built Kernel blueprint id, reusing an existing one unless rebuild=True.
