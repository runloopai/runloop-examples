 ...
 */ export class PGVectorStore extends BaseVectorStore {
   ...
   */ async delete(refDocId, deleteKwargs) {
    ...
    buildFilterClause(filter, paramIndex) {
        ...
        if (filter.operator === FilterOperator.ALL || filter.operator === FilterOperator.ANY) {
            return {
        ...
        if (filter.operator === FilterOperator.TEXT_MATCH) {
            const escapedValue = escapeLikeString(filter.value);
            return {
   ...
   */ async query(query, options) {
        ...
        if (filterClauses.length > 0) {
            const condition = this.toPostgresCondition(query.filters?.condition ?? FilterCondition.AND);
