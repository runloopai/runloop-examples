...
class KVDocumentStore extends _docstore.BaseDocumentStore {
    kvstore;
    nodeCollection;
    refDocCollection;
    metadataCollection;
    ...
    async docs() {
        ...
        for(const key in jsonDict){
            ...
            if ((0, _docstore.isValidDocJson)(value)) {
                docs[key] = (0, _docstore.jsonToDoc)(value, this.serializer);
            } else {
    ...
    async addDocuments(docs, allowUpdate = true) {
        for(let idx = 0; idx < docs.length; idx++){
            ...
            const data = (0, _docstore.docToJson)(doc, this.serializer);
            await this.kvstore.put(nodeKey, data, this.nodeCollection);
            const metadata = {
                docHash: doc.hash
            };
            if (doc.type === _schema.ObjectType.TEXT && doc.sourceNode !== undefined) {
    ...
    async getDocument(docId, raiseError = true) {
        ...
        if (!(0, _docstore.isValidDocJson)(json)) {
            throw new Error(`Invalid JSON for docId ${docId}`);
        }
        return (0, _docstore.jsonToDoc)(json, this.serializer);
    }
