...
const _decorator = require("@llamaindex/core/decorator");
...
const GEMINI_MODEL_INFO_MAP = {
    ...
    [_types.GEMINI_MODEL.GEMINI_PRO_FLASH_LATEST]: {
        contextWindow: 10 ** 6
    },
    ...
    [_types.GEMINI_MODEL.GEMINI_PRO_1_5_LATEST]: {
        contextWindow: 2 * 10 ** 6
    },
    [_types.GEMINI_MODEL.GEMINI_PRO_1_5_FLASH_LATEST]: {
        contextWindow: 10 ** 6
    },
    [_types.GEMINI_MODEL.GEMINI_2_0_FLASH_EXPERIMENTAL]: {
        contextWindow: 10 ** 6
    }
};
...
class GeminiSessionStore {
    ...
    static getSessionId(options) {
        if (options.backend === _types.GEMINI_BACKENDS.GOOGLE) {
...
class Gemini extends _llms.ToolCallLLM {
    static{
        ({ e: [_initProto] } = _apply_decs_2203_r(this, [
            [
                _decorator.wrapLLMEvent,
                2,
                "chat"
            ]
        ], []));
    ...
    async nonStreamChat(params) {
        ...
        const chat = client.startChat(params.tools ? {
            history: context.history,
            tools: [
                {
                    functionDeclarations: params.tools.map(_utils1.mapBaseToolToGeminiFunctionDeclaration)
                }
            ],
            safetySettings: _utils1.DEFAULT_SAFETY_SETTINGS
        } : {
    ...
    async complete(params) {
        ...
        const client = this.session.getGenerativeModel(this.metadata);
        if (stream) {
            const result = await client.generateContentStream((0, _utils1.getPartsText)(_utils1.GeminiHelper.messageContentToGeminiParts({
                content: prompt
            })));
            return this.session.getCompletionStream(result);
        }
        const result = await client.generateContent((0, _utils1.getPartsText)(_utils1.GeminiHelper.messageContentToGeminiParts({
            content: prompt
        })));
        return {
