...
_export(exports, {
    ...
    PGVECTOR_SCHEMA: function() {
        return PGVECTOR_SCHEMA;
    },
    PGVECTOR_TABLE: function() {
        return PGVECTOR_TABLE;
    },
...
function fromVercelPool(client) {
...
class PGVectorStore extends _types.BaseVectorStore {
    storesText = true;
    collection = _global.DEFAULT_COLLECTION;
    ...
    constructor(config){
        ...
        if ("clientConfig" in config) {
            ...
            if (config.client.constructor.name.includes("Vercel") || !!config.client.connect && !!config.client.query && !config.client.unsafe) {
                this.isDBConnected = true;
                this.db = fromVercelPool(config.client);
   ...
   */ getCollection() {
    ...
    async checkSchema(db) {
      ...
      embeddings VECTOR(${this.dimensions})
        ...
        await db.query(`CREATE INDEX IF NOT EXISTS idx_${this.tableName}_external_id ON ${this.schemaName}.${this.tableName} (external_id);`);
        await db.query(`CREATE INDEX IF NOT EXISTS idx_${this.tableName}_collection ON ${this.schemaName}.${this.tableName} (collection);`);
   ...
   */ async clearCollection() {
        const sql = `DELETE FROM ${this.schemaName}.${this.tableName}
                         WHERE collection = $1`;
    ...
    getDataToInsert(embeddingResults) {
        return embeddingResults.map((node)=>{
            ...
            return [
                ...
                "",
                this.collection,
                node.getContent(_schema.MetadataMode.NONE),
                meta,
   ...
   */ async add(embeddingResults) {
        ...
        return db.begin(async (query)=>{
            ...
            const placeholders = data.map((_, index)=>`($${index * 6 + 1}, ` + `$${index * 6 + 2}, ` + `$${index * 6 + 3}, ` + `$${index * 6 + 4}, ` + `$${index * 6 + 5}, ` + `$${index * 6 + 6})`).join(", ");
            const sql = `
   ...
   */ async delete(refDocId, deleteKwargs) {
    ...
    toPostgresCondition(condition) {
        if (condition === _types.FilterCondition.AND) {
            return "AND";
        }
        if (condition === _types.FilterCondition.OR) {
            return "OR";
        } else {
            return "AND";
        }
    }
    toPostgresOperator(operator) {
        if (operator === _types.FilterOperator.EQ) {
            return "=";
        }
        if (operator === _types.FilterOperator.GT) {
            return ">";
        }
        if (operator === _types.FilterOperator.LT) {
            return "<";
        }
        if (operator === _types.FilterOperator.NE) {
            return "!=";
        }
        if (operator === _types.FilterOperator.GTE) {
            return ">=";
        }
        if (operator === _types.FilterOperator.LTE) {
            return "<=";
        }
        if (operator === _types.FilterOperator.IN) {
            return "= ANY";
        }
        if (operator === _types.FilterOperator.NIN) {
            return "!= ANY";
        }
        if (operator === _types.FilterOperator.CONTAINS) {
            return "@>";
        }
        if (operator === _types.FilterOperator.ANY) {
            return "?|";
        }
        if (operator === _types.FilterOperator.ALL) {
            return "?&";
        }
        // fallback to "="
        return "=";
    }
    buildFilterClause(filter, paramIndex) {
        if (filter.operator === _types.FilterOperator.IN || filter.operator === _types.FilterOperator.NIN) {
            return {
                clause: `metadata->>'${filter.key}' ${this.toPostgresOperator(filter.operator)}($${paramIndex})`,
                param: filter.value
            };
        }
        if (filter.operator === _types.FilterOperator.ALL || filter.operator === _types.FilterOperator.ANY) {
            return {
                clause: `metadata->'${filter.key}' ${this.toPostgresOperator(filter.operator)} $${paramIndex}::text[]`,
                param: filter.value
            };
        }
        if (filter.operator === _types.FilterOperator.CONTAINS) {
            return {
        ...
        if (filter.operator === _types.FilterOperator.TEXT_MATCH) {
            const escapedValue = (0, _utils.escapeLikeString)(filter.value);
            return {
   ...
   */ async query(query, options) {
        ...
        const whereClauses = this.collection.length ? [
            "collection = $2"
        ] : [];
        const params = this.collection.length ? [
            embedding,
        ...
        if (filterClauses.length > 0) {
            const condition = this.toPostgresCondition(query.filters?.condition ?? _types.FilterCondition.AND);
    ...
    `;
        ...
        const nodes = results.map((row)=>{
            return new _schema.Document({
                ...
                embedding: typeof row.embeddings === "string" ? JSON.parse(row.embeddings) : row.embeddings
            });
        });
        const ret = {
            nodes: nodes,
