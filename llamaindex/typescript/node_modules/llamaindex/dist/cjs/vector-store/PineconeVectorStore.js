...
class PineconeVectorStore extends _types.BaseVectorStore {
    storesText = true;
    /*
  ...
  */ db;
    ...
    constructor(params){
        ...
        const apiKey = params?.apiKey ?? (0, _env.getEnv)("PINECONE_API_KEY");
        if (!apiKey) {
   ...
   */ persist(persistPath) {
    ...
    toPineconeFilter(stdFilters) {
        ...
        const transformCondition = (condition = "and")=>{
        ...
        const convertFilterItem = (filter)=>{
            return {
                [filter.key]: {
        ...
        const convertFilter = (filter)=>{
            const filtersList = filter.filters.map((f)=>convertFilterItem(f)).filter((f)=>Object.keys(f).length > 0);
