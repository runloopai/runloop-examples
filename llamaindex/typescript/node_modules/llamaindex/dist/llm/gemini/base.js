...
import { DEFAULT_SAFETY_SETTINGS, GeminiHelper, getChatContext, getPartsText, mapBaseToolToGeminiFunctionDeclaration } from "./utils.js";
 ...
 */ export class Gemini extends ToolCallLLM {
    ...
    async complete(params) {
        ...
        const client = this.session.getGenerativeModel(this.metadata);
        if (stream) {
            const result = await client.generateContentStream(getPartsText(GeminiHelper.messageContentToGeminiParts({
                content: prompt
            })));
            return this.session.getCompletionStream(result);
        }
        const result = await client.generateContent(getPartsText(GeminiHelper.messageContentToGeminiParts({
            content: prompt
        })));
        return {
