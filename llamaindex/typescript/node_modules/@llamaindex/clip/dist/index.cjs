...
async function readImage(input) {
    const { RawImage } = await multiModel.loadTransformers((transformer)=>{
...
class ClipEmbedding extends embeddings.MultiModalEmbedding {
    ...
    async getProcessor() {
        const { AutoProcessor } = await multiModel.loadTransformers((transformer)=>{
            global.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });
    ...
    async getVisionModel() {
        const { CLIPVisionModelWithProjection } = await multiModel.loadTransformers((transformer)=>{
            global.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });
    ...
    async getTextModel() {
        const { CLIPTextModelWithProjection } = await multiModel.loadTransformers((transformer)=>{
            global.Settings.callbackManager.dispatchEvent("load-transformers", {
                transformer
            }, true);
        });
